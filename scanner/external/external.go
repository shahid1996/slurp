...
func PermutateDomain(domain, suffix, cfgPermutationsFile string) []string {
	...
	for i := range perms {
		log.Debugf(perms[i].(string), domain, s3url)
	...
	log.Debugf("%s.%s.%s", domain, suffix, s3url)
	log.Debugf("%s.%s", strings.Replace(fmt.Sprintf("%s.%s", domain, suffix), ".", "", -1), s3url)
	...
	log.Infof("Generated permutations in: %s", delta)
...
func PermutateKeyword(keyword, cfgPermutationsFile string) []string {
	...
	// Our list of permutations
	for i := range perms {
		log.Debugf(perms[i].(string), keyword, s3url)
	...
	for {
		...
		go func(pd PermutatedDomain) {
			...
			if resp.StatusCode == 200 {
				...
				cfg.Stats.IncRequests307()
				cfg.Stats.Add307Link(pd.Permutation)
...
func Init(cfg *cmd.Config) {
	...
	tr := &http.Transport{
		...
		MaxIdleConnsPerHost:    cfg.Concurrency * cfg.Concurrency,
		MaxIdleConns:           100,
